#!/usr/bin/env python3
"""
Inicializa la base de datos SQLite de Fachmann con el schema y datos de ejemplo.
Ejecutar una sola vez: python src/setup_db.py
"""
...
def insert_sample_data(conn: sqlite3.Connection) -> None:
    ...
    conn.executemany(
        """INSERT OR IGNORE INTO productos_catalogo
    ...
    conn.executemany(
        """INSERT OR IGNORE INTO clientes_prospectos
    ...
    conn.executemany(
        """INSERT OR IGNORE INTO oportunidades_ventas
    ...
    conn.executemany(
        "INSERT OR IGNORE INTO interacciones (cliente_id, tipo, notas, fecha) VALUES (?, ?, ?, ?)",
        interacciones,
    )
